...
// Node object
function Node(id, pid, name, url, level,src, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
dTree.prototype.node = function(node, nodeId) {	console.log(node.id);
	...
	if (node.url) {
		str += '<a id="s' + this.obj + nodeId + '" class="' + ((this.config.useSelection) ? ((node._is ? 'nodeSel' : 'node')) : 'node') + '" href="#" onclick="show_sub(\''+node.name+'\',\''+node.url+'\','+node.pid+','+node.level+','+node.id+',\''+node.src+'\',this)"';
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
			str += '<a href="javascript: ' + this.obj + '.o(' + nodeId + ',\''+node.url+'\');"><img id="j' + this.obj + nodeId + '" src="';
...
dTree.prototype.o = function(id,url) {	var cn = this.aNodes[id];
//	if(cn._io)//		show_sub(url,id);
